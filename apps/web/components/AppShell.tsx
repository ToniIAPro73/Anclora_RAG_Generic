...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[var(--background)] text-[var(--foreground)] transition-colors">
      <header className={`shadow-lg transition-opacity ${isHydrated ? 'opacity-100' : 'opacity-75'}`}>
        <div className="container-app py-3">
          <div className="flex items-center justify-between gap-4">
            ...
            <nav
              className={`flex flex-wrap justify-center gap-2 flex-1 ${
                isHydrated ? '' : 'pointer-events-none opacity-60'
              }`}
            >
      ...
      <main className={isHydrated ? undefined : 'pointer-events-none opacity-0'}>{children}</main>
    </div>
  );
}
