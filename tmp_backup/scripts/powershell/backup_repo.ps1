...
if ($DumpDocker) {
    ...
    return $LASTEXITCODE
        ...
        $result = Invoke-DockerComposeCommand @("exec","-T","postgres","bash","-lc", "pg_dumpall -U anclora_user > $pgTemp")
        if ($result -eq 0) {
            $pgFile = Join-Path $dockerDir "postgres.sql"
            $copyResult = Copy-FromContainer "postgres" $pgTemp $pgFile
            if ($copyResult -eq 0) {
                $null = Invoke-DockerComposeCommand @("exec","-T","postgres","rm","-f",$pgTemp)
                $dockerMeta.postgres = "docker/postgres.sql"
            }
        }

        $qdTemp = "/tmp/anclora_qdrant.tar.gz"
        $result = Invoke-DockerComposeCommand @("exec","-T","qdrant","bash","-lc","tar czf $qdTemp -C / qdrant/storage")
        if ($result -eq 0) {
            $qdFile = Join-Path $dockerDir "qdrant_storage.tar.gz"
            $copyResult = Copy-FromContainer "qdrant" $qdTemp $qdFile
            if ($copyResult -eq 0) {
                $null = Invoke-DockerComposeCommand @("exec","-T","qdrant","rm","-f",$qdTemp)
                $dockerMeta.qdrant = "docker/qdrant_storage.tar.gz"
            }
        }

        $redisTemp = "/tmp/anclora_redis.tar.gz"
        $result = Invoke-DockerComposeCommand @("exec","-T","redis","bash","-lc","tar czf $redisTemp -C / data")
        if ($result -eq 0) {
            $redisFile = Join-Path $dockerDir "redis_data.tar.gz"
            $copyResult = Copy-FromContainer "redis" $redisTemp $redisFile
            if ($copyResult -eq 0) {
                $null = Invoke-DockerComposeCommand @("exec","-T","redis","rm","-f",$redisTemp)
                $dockerMeta.redis = "docker/redis_data.tar.gz"
            }
        }

        $ollamaTemp = "/tmp/anclora_ollama.tar.gz"
        $result = Invoke-DockerComposeCommand @("exec","-T","ollama","bash","-lc","tar czf $ollamaTemp -C /root .ollama")
        if ($result -eq 0) {
            $ollamaFile = Join-Path $dockerDir "ollama_data.tar.gz"
            $copyResult = Copy-FromContainer "ollama" $ollamaTemp $ollamaFile
            if ($copyResult -eq 0) {
                $null = Invoke-DockerComposeCommand @("exec","-T","ollama","rm","-f",$ollamaTemp)
